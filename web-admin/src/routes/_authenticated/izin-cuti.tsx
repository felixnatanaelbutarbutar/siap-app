            ...
            {izinList?.map((item: any) => (
              <tr key={item.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                ...
                <td className="px-5 py-3.5 text-sm text-slate-700 font-medium">
                  {format(new Date(item.tanggal_mulai), 'dd MMM yyyy', { locale: id })} <span className="text-slate-600">s/d</span> {format(new Date(item.tanggal_selesai), 'dd MMM yyyy', { locale: id })}
                </td>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              ...
              <button onClick={() => setSelectedIzin(null)} className="text-slate-600 hover:text-slate-600 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Jenis Izin</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Status Saat Ini</label>
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Keterangan / Alasan</label>
              ...
              {selectedIzin.foto_url && (
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Lampiran Bukti</label>
                  <a 
